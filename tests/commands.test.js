...
// which Gemini CLI reuses), OpenCode (.opencode/command/*.md), and Kimi
// (.kimi/commands/*.md). /ponytail-help was advertised in the README and the
...
test('every registered command ships a Kimi .kimi/commands/*.md', () => {
  for (const name of commands) {
    assert.ok(
      fs.existsSync(path.join(root, '.kimi', 'commands', `${name}.md`)),
      `missing .kimi/commands/${name}.md`,
    );
  }
});
